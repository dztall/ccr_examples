//Original source code found at : http://www.lemoda.net/c/zlib-open-write/
...
#define FILE_NAME "compressed.dat"
...
#define CALL_ZLIB(x) {											\
	...
	status = x;													\
	if (status < 0) {											\
		printf ( "%s:%d: %s returned a bad status of %d.\n",	\
...
int main()
{
	...
	do {
		...
		fwrite(out, sizeof(char), have, fp);
