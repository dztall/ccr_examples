//Original source code found at : https://gist.github.com/PhirePhly/3080633#file-memdjpeg-c
...
int main(int argc, char *argv[]) {
	...
	i = 0;
	while (i < jpg_size) {
		rc = fread(jpg_buffer + i, 1, jpg_size - i, fp);
	...
	fwrite(buf, 1, rc, fp); // Write the PPM image header before data
	fwrite(bmp_buffer, 1, bmp_size, fp); // Write out all RGB pixel data
